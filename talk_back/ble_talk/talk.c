...
void ble_talk_downlink(uint8_t actv_idx, uint8_t *data, uint16_t data_len)
{
    ...
    */
    ...
    // 检查当前设备是否设置了房间标识，未设置则拒绝接收音频（依赖驱动网络房间ID）
    const char *room_id = ble_talk_network_get_room_id();
    if (!room_id || rt_strnlen(room_id, ROOM_ID_LEN) == 0)
    {
        rt_kprintf("房间ID未设置，忽略音频数据\n");
        return;
    }
...
int talk_init(audio_rwflag_t flag)
{
    ...
    thiz = g_talker;
    thiz->flag = flag;
    /* 修正标志映射：AUDIO_TX 开启发送；AUDIO_RX 开启接收 */
    if (flag & AUDIO_TX)
    {
        thiz->is_tx_enable = 1;
    }
...
{
    return (g_talker && g_talker->is_rx_enable) ? 1 : 0;
}
